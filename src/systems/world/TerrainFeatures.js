...
export class TerrainFeatures {
 ...
 buildIntegratedCliff(){
  ...
  for(let i=0;i<anchors.length;i++){
   ...
   const baseYaw=this.yawForProfile(profile)+(this.rand(i*47+9)-.5)*.07,curve=this.curvatureAt(t0),absCurve=Math.abs(curve),targetWidth=4.0+this.rand(seed+2)*1.4;
   if(end)this.placeFalloff(this.prototypes.falloff.edge,profile,anchorX,anchorZ,baseYaw+(i===0?Math.PI:0),targetWidth*1.05,seed);
   else if(profile.drop<1.6&&this.rand(seed+4)>.18)this.placeFalloff(this.prototypes.falloff.center,profile,anchorX,anchorZ,baseYaw,targetWidth*1.02,seed+6);
   ...
   else if(this.rand(seed+12)<.06)this.placeFalloff(this.prototypes.falloff.center,profile,anchorX,anchorZ,baseYaw,targetWidth*.96,seed+20);
   else this.placeStack(this.rand(seed+16)<.32?this.prototypes.escarp:this.prototypes.cliff,profile,anchorX,anchorZ,baseYaw,targetWidth,seed+30,false);

   if(this.rand(seed+50)>.22){const rx=profile.x-profile.nx*(1.4+this.rand(seed+51)*2.3)+(this.rand(seed+52)-.5)*2.4*profile.tx,rz=profile.z-profile.nz*(1.4+this.rand(seed+53)*2.3)+(this.rand(seed+54)-.5)*2.4*profile.tz;this.addNatural(this.prototypes.rocks,rx,this.world.heightAt(rx,rz)-.07,rz,this.rand(seed+55)*Math.PI*2,1+this.rand(seed+56)*1.5);}
   if(this.rand(seed+60)>.48){const bx=profile.x+profile.nx*(.75+this.rand(seed+61)*1.5)+(this.rand(seed+62)-.5)*2.15*profile.tx,bz=profile.z+profile.nz*(.75+this.rand(seed+63)*1.5)+(this.rand(seed+64)-.5)*2.15*profile.tz;this.addNatural(this.prototypes.bushes,bx,this.world.heightAt(bx,bz),bz,this.rand(seed+65)*Math.PI*2,1.25+this.rand(seed+66)*1.3);}
  }
 }
