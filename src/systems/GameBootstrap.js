import { WorldManager } from './WorldManager.js?v=543';
...
export class GameBootstrap {
 ...
 start(){
  ...
  try{
   ...
   if(status)status.textContent='Clean rebuild 0.5.43 · natural cliff breakup · Ranger loading';
   const loop=()=>{
   ...
   if(status){status.textContent='0.5.43 STARTUP ERROR: '+(err?.message||err);status.style.background='#5b1818';}
  }
 }
 ...
 async tryKayKitRanger(status){
  ...
  try{
   ...
   if(status)status.textContent=ranger.actions.size
    ?'Clean rebuild 0.5.43 · Ranger · natural cliff breakup'
    :'Clean rebuild 0.5.43 · Ranger · natural cliff breakup · animations pending';
  }catch(err){
   console.error('[KayKit Ranger model load]',err);
   if(status)status.textContent='Clean rebuild 0.5.43 · natural cliff breakup · Ranger model unavailable';
  }
 }
}
