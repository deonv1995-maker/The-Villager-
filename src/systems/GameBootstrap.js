...
export class GameBootstrap{
 ...
 start(){
  ...
  this.renderer=new T.WebGLRenderer({antialias:true,powerPreference:'high-performance'});this.renderer.setPixelRatio(Math.min(devicePixelRatio||1,1.5));this.renderer.setSize(innerWidth,innerHeight);document.body.appendChild(this.renderer.domElement);
  ...
  const loop=()=>{requestAnimationFrame(loop);const dt=Math.min(this.clock.getDelta(),.05);this.playerController.update(dt);this.playerVisual.update(dt,this.playerController.isMoving);this.cameraController.update(dt);this.renderer.render(this.scene,this.camera);};loop();
 }
}
