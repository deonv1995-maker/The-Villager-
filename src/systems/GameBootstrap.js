import { WorldManager } from './WorldManager.js?v=517';
...
export class GameBootstrap {
 ...
 start(){
  ...
  try{
   ...
   this.renderer=new T.WebGLRenderer({antialias:true,powerPreference:'high-performance'});this.renderer.setPixelRatio(Math.min(devicePixelRatio||1,1.5));this.renderer.setSize(innerWidth,innerHeight);document.body.insertBefore(this.renderer.domElement,document.body.firstChild);
   ...
   this.playerVisual=new PlayerVisual(T);this.player.add(this.playerVisual.root);
   ...
   this.cameraController=new ThirdPersonCamera(T,{camera:this.camera,target:this.player,input:this.input});this.playerController=new PlayerController(T,{player:this.player,input:this.input,cameraController:this.cameraController,world:this.world,groundOffset:0});this.cameraController.update(1/60);
   addEventListener('resize',()=>{this.camera.aspect=innerWidth/innerHeight;this.camera.updateProjectionMatrix();this.renderer.setSize(innerWidth,innerHeight);});
   if(status)status.textContent='Clean rebuild 0.5.17 · terrain-aligned cliffs · Ranger loading';
  ...
  }catch(err){console.error('[BOOT]',err);if(status){status.textContent='0.5.17 STARTUP ERROR: '+(err?.message||err);status.style.background='#5b1818';}}
 }
 async tryKayKitRanger(status){
  const T=this.THREE;
  try{
   const {KayKitPlayerVisual}=await import('./player/KayKitPlayerVisual.js?v=517');
   ...
   await ranger.load();const old=this.playerVisual;this.player.add(ranger.root);if(old?.root?.parent===this.player)this.player.remove(old.root);this.playerVisual=ranger;
   if(status)status.textContent=ranger.actions.size?'Clean rebuild 0.5.17 · Ranger · terrain-aligned cliffs':'Clean rebuild 0.5.17 · Ranger · terrain-aligned cliffs · animations pending';
  }catch(err){console.error('[KayKit Ranger model load]',err);if(status)status.textContent='Clean rebuild 0.5.17 · terrain-aligned cliffs · Ranger model unavailable';}
 }
}
