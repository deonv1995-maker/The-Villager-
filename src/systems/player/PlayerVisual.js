export class PlayerVisual{
 ...
 build(){const T=this.T;const skin=this.mat(0xd99a62),shirt=this.mat(0x4f7d55),leather=this.mat(0x5b3926),dark=this.mat(0x263229),boot=this.mat(0x3d2a22),hair=this.mat(0x5a3828);
 ...
 update(dt,moving){if(!moving){this.phase=0;this.leftArm.rotation.x*=.82;this.rightArm.rotation.x*=.82;this.leftLeg.rotation.x*=.82;this.rightLeg.rotation.x*=.82;return;}this.phase+=dt*9;const s=Math.sin(this.phase)*.65;this.leftArm.rotation.x=s;this.rightArm.rotation.x=-s;this.leftLeg.rotation.x=-s*.65;this.rightLeg.rotation.x=s*.65;this.body.position.y=1.35+Math.abs(Math.sin(this.phase))*0.035;}
}
