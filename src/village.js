...
function addFlowerPatch(scene, x, y, count, spreadX, spreadY, depth) {
  for (let i = 0; i < count; i += 1) {
    ...
    const stem = scene.add.rectangle(px, py + 4, 2, 8, 0x4f7f3d, 1).setDepth(depth);
...
function drawPathStones(scene, x, y, horizontal, length, depth) {
  ...
  for (let offset = -length / 2 + 18, i = 0; offset < length / 2; offset += step, i += 1) {
    ...
    const stone = scene.add.ellipse(px, py, 13 + (i % 3) * 2, 7 + (i % 2) * 2, 0x9c8767, 0.42);
...
function drawBuilding(scene, building) {
  ...
  roof.beginPath();
  roof.moveTo(-building.width * 0.61, -building.height * 0.12);
  roof.lineTo(0, -building.height * 0.64);
  roof.lineTo(building.width * 0.61, -building.height * 0.12);
  roof.lineTo(building.width * 0.48, building.height * 0.03);
  roof.lineTo(-building.width * 0.48, building.height * 0.03);
  roof.closePath();
  roof.fillPath();
  roof.strokePath();

  const roofHighlight = scene.add.graphics();
  roofHighlight.lineStyle(5, building.roofLight, 0.7);
  roofHighlight.beginPath();
  roofHighlight.moveTo(-building.width * 0.47, -building.height * 0.13);
  roofHighlight.lineTo(0, -building.height * 0.55);
  roofHighlight.strokePath();

  const chimney = scene.add.rectangle(building.width * 0.27, -building.height * 0.45, 14, 30, 0x79604c, 1);
  chimney.setStrokeStyle(2, 0x4d3b30, 1);
  const chimneyCap = scene.add.rectangle(building.width * 0.27, -building.height * 0.61, 19, 5, 0x4d3b30, 1);

  const door = scene.add.rectangle(0, building.height * 0.21, 25, building.height * 0.37, 0x5a3826);
  ...
  const leftWindow = scene.add.rectangle(-building.width * 0.27, windowY, 23, 20, 0x9fcbd0);
  const rightWindow = scene.add.rectangle(building.width * 0.27, windowY, 23, 20, 0x9fcbd0);
  leftWindow.setStrokeStyle(3, building.timber, 1);
  rightWindow.setStrokeStyle(3, building.timber, 1);
  const leftGlint = scene.add.rectangle(-building.width * 0.27 - 4, windowY - 4, 5, 8, 0xd9f0e7, 0.75);
  const rightGlint = scene.add.rectangle(building.width * 0.27 - 4, windowY - 4, 5, 8, 0xd9f0e7, 0.75);
...
function drawVillageProps(scene) {
  ...
  drawFence(scene, 694, 421, 176, depth);
  drawFence(scene, 1106, 421, 176, depth);
  ...
  const crate1 = scene.add.rectangle(650, 711, 26, 24, 0x9a6d43, 1).setDepth(724);
  const crate2 = scene.add.rectangle(675, 718, 23, 21, 0x845b39, 1).setDepth(725);
  crate1.setStrokeStyle(2, 0x563b28, 1);
  crate2.setStrokeStyle(2, 0x563b28, 1);
  scene.add.line(650, 711, -9, -8, 9, 8, 0x5d412b, 0.7).setDepth(725);
  scene.add.line(675, 718, -8, 7, 8, -7, 0x5d412b, 0.7).setDepth(726);

  const barrel = scene.add.ellipse(1130, 714, 25, 31, 0x765239, 1).setDepth(728);
  barrel.setStrokeStyle(3, 0x4a3528, 1);
  scene.add.rectangle(1130, 708, 27, 4, 0x3f3b36, 0.9).setDepth(729);
  scene.add.rectangle(1130, 720, 27, 4, 0x3f3b36, 0.9).setDepth(729);

  const signPost = scene.add.rectangle(820, 554, 7, 42, 0x765139, 1).setDepth(575);
  const sign = scene.add.rectangle(832, 538, 42, 19, 0x9c754f, 1).setDepth(576);
  sign.setStrokeStyle(2, 0x4f392a, 1);
  signPost.setStrokeStyle(1, 0x4f392a, 1);

  const benchSeat = scene.add.rectangle(974, 670, 56, 8, 0x805638, 1).setDepth(680);
  const benchBack = scene.add.rectangle(974, 659, 56, 7, 0x906445, 1).setDepth(679);
  benchSeat.setStrokeStyle(1, 0x4c3526, 1);
  benchBack.setStrokeStyle(1, 0x4c3526, 1);
...
export function createVillage(scene) {
  ...
  const boundaryShadow = scene.add.ellipse(
    VILLAGE_CONFIG.centerX,
    VILLAGE_CONFIG.centerY + 30,
    VILLAGE_CONFIG.width + 22,
    VILLAGE_CONFIG.height + 28,
    0x20351f,
    0.16,
  );
  boundaryShadow.setDepth(backDepth - 2);
  ...
  boundary.setStrokeStyle(5, 0x526f42, 0.82);
  boundary.setDepth(backDepth - 1);

  const villageInner = scene.add.rectangle(
    VILLAGE_CONFIG.centerX,
    VILLAGE_CONFIG.centerY,
    VILLAGE_CONFIG.width - 24,
    VILLAGE_CONFIG.height - 24,
    0x84a765,
    0.18,
  );
  villageInner.setStrokeStyle(2, 0x9abc77, 0.22);
  villageInner.setDepth(backDepth);

  const mainPathShadow = scene.add.rectangle(VILLAGE_CONFIG.centerX + 3, 603, 82, 394, 0x655744, 0.28);
  const crossPathShadow = scene.add.rectangle(VILLAGE_CONFIG.centerX + 3, 613, 526, 73, 0x655744, 0.28);
  mainPathShadow.setDepth(backDepth + 0.5);
  crossPathShadow.setDepth(backDepth + 0.5);

  const mainPath = scene.add.rectangle(VILLAGE_CONFIG.centerX, 600, 72, 390, 0xbca478, 0.97);
  const crossPath = scene.add.rectangle(VILLAGE_CONFIG.centerX, 610, 520, 64, 0xbca478, 0.97);
  mainPath.setStrokeStyle(2, 0x9b825e, 0.8);
  crossPath.setStrokeStyle(2, 0x9b825e, 0.8);
  mainPath.setDepth(backDepth + 1);
  crossPath.setDepth(backDepth + 1);

  drawPathStones(scene, 900, 600, false, 370, backDepth + 2);
  drawPathStones(scene, 900, 610, true, 495, backDepth + 2);

  const greenShadow = scene.add.ellipse(VILLAGE_CONFIG.centerX + 3, 614, 204, 140, 0x3f6237, 0.33);
  greenShadow.setDepth(backDepth + 2.4);
  const green = scene.add.ellipse(VILLAGE_CONFIG.centerX, 610, 194, 132, 0x75a75c, 1);
  green.setStrokeStyle(5, 0x527941, 1);
  green.setDepth(backDepth + 2.5);

  const wellShadow = scene.add.ellipse(VILLAGE_CONFIG.centerX + 2, 617, 66, 39, 0x32402f, 0.35);
  wellShadow.setDepth(612);
  const wellBase = scene.add.ellipse(VILLAGE_CONFIG.centerX, 610, 60, 37, 0x857d70, 1);
  wellBase.setStrokeStyle(5, 0x524c44, 1);
  wellBase.setDepth(614);
  const wellLip = scene.add.ellipse(VILLAGE_CONFIG.centerX, 606, 48, 28, 0xa9a094, 1);
  wellLip.setStrokeStyle(3, 0x625d55, 1);
  wellLip.setDepth(615);
  const wellWater = scene.add.ellipse(VILLAGE_CONFIG.centerX, 604, 34, 16, 0x5b9ab3, 1);
  wellWater.setDepth(616);
  scene.add.ellipse(VILLAGE_CONFIG.centerX - 7, 601, 10, 4, 0xb7dde3, 0.5).setDepth(617);
  ...
  wellPostLeft.setStrokeStyle(1, 0x3f2e22, 1);
  wellPostRight.setStrokeStyle(1, 0x3f2e22, 1);
  wellBeam.setStrokeStyle(1, 0x3f2e22, 1);
  ...
  const wellBlocker = scene.add.ellipse(VILLAGE_CONFIG.centerX, 610, 52, 31, 0x000000, 0);
