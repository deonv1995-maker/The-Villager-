...
function textImage(path){const img=new Image();img.decoding='async';fetch(new URL(path,import.meta.url).href,{cache:'no-store'}).then(r=>r.text()).then(b64=>{img.src=`data:image/png;base64,${b64.trim()}`;}).catch(()=>{});return img;}
...
export function drawGround(ctx,originX,originY,width,height){ctx.fillStyle='#355f2c';ctx.fillRect(0,0,width,height);if(!ready(ASSETS.ground))return;const tile=384,sx=((originX%tile)+tile)%tile,sy=((originY%tile)+tile)%tile;for(let y=sy-tile;y<height+tile;y+=tile){for(let x=sx-tile;x<width+tile;x+=tile){const gx=Math.floor((x-originX)/tile),gy=Math.floor((y-originY)/tile),flipX=hash(gx,gy,2)>.5,flipY=hash(gx,gy,3)>.5,rotate=hash(gx,gy,4)>.72;ctx.save();ctx.translate(x+tile/2,y+tile/2);if(rotate)ctx.rotate(Math.PI);ctx.scale(flipX?-1:1,flipY?-1:1);ctx.drawImage(ASSETS.ground,-tile/2,-tile/2,tile,tile);ctx.restore();}}}
export function drawResource(ctx,node){if(node.type==='tree')return drawTree(ctx,node);if(node.type==='rock')return drawRock(ctx,node);return drawGrass(ctx,node);}
function drawTree(ctx,node){ellipse(ctx,node.x+6,node.y+20,70,18,'#0d150d',.34);if(!ready(ASSETS.tree))return;const scale=1.02+hash(node.x,node.y,4)*.18,w=175*scale,h=252*scale,flip=hash(node.x,node.y,5)>.5;ctx.save();ctx.translate(node.x,node.y);if(flip)ctx.scale(-1,1);ctx.drawImage(ASSETS.tree,-w/2,-h+28,w,h);ctx.restore();}
function drawRock(ctx,node){ellipse(ctx,node.x,node.y+13,43,12,'#0e1210',.28);if(!ready(ASSETS.rock))return;const scale=.84+hash(node.x,node.y,7)*.24,w=124*scale,h=100*scale,flip=hash(node.x,node.y,8)>.5;ctx.save();ctx.translate(node.x,node.y);if(flip)ctx.scale(-1,1);ctx.drawImage(ASSETS.rock,-w/2,-h+18,w,h);ctx.restore();}
