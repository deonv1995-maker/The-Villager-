...
function textImage(key){
  ...
  fetch(assetUrl(key),{cache:'no-store'})
    ...
    .catch(error=>{img.dataset.loadState='error';console.error('Asset load failed:',key,error);});
  return img;
}
...
// v0.5.8 uses a single clean six-frame strip. The source art is never sliced
// across neighbouring sprites; left-facing movement is mirrored at render time.
const PLAYER_SHEET={frameWidth:64,frameHeight:64,columns:6,drawWidth:104,drawHeight:104,walkFps:8};
...
function drawTree(ctx,node){
  ...
  ctx.save();ctx.translate(node.x,node.y);if(flip)ctx.scale(-1,1);ctx.drawImage(ASSETS.tree,-w/2,-h+28,w,h);ctx.restore();
}
...
function drawRock(ctx,node){
  ...
  ctx.save();ctx.translate(node.x,node.y);if(flip)ctx.scale(-1,1);ctx.drawImage(ASSETS.rock,-w/2,-h+18,w,h);ctx.restore();
}
...
function drawGrass(ctx,node){
  ...
  ctx.save();ctx.translate(node.x,node.y);if(flip)ctx.scale(-1,1);ctx.drawImage(ASSETS.grass,-w/2,-h+15,w,h);ctx.restore();
}

function playerFrame(player,t){
  return player.state==='walk'?Math.floor(t*PLAYER_SHEET.walkFps)%PLAYER_SHEET.columns:0;
}

function drawPlayerAssetError(ctx,x,y){
  ctx.save();ctx.fillStyle='#4d1111';ctx.fillRect(x-28,y-62,56,56);ctx.strokeStyle='#ffcf5a';ctx.lineWidth=3;ctx.strokeRect(x-28,y-62,56,56);ctx.fillStyle='#fff3c4';ctx.font='bold 9px monospace';ctx.textAlign='center';ctx.fillText('PLAYER',x,y-34);ctx.fillText('ASSET ERROR',x,y-22);ctx.restore();
}
...
export function drawPlayer(ctx,player,gameTime,crafting){
  ...
  if(ready(ASSETS.playerSheet)){
    const col=playerFrame(player,gameTime),sx=col*PLAYER_SHEET.frameWidth;
    const flip=player.facingX<-.2;
    ctx.translate(x,y);
    if(flip)ctx.scale(-1,1);
    ctx.drawImage(ASSETS.playerSheet,sx,0,PLAYER_SHEET.frameWidth,PLAYER_SHEET.frameHeight,-PLAYER_SHEET.drawWidth/2,-PLAYER_SHEET.drawHeight+20,PLAYER_SHEET.drawWidth,PLAYER_SHEET.drawHeight);
...
function drawTool(ctx,player,t,crafting){
  ...
  ctx.save();ctx.translate(player.x+dir*20,player.y-38);ctx.rotate(swing);rect(ctx,-2,-1,5,7,TOOL.skin);rect(ctx,0,3,4,32,TOOL.leather);rect(ctx,1,5,2,27,TOOL.leatherDark);
  if(!equipped)rect(ctx,-1,29,6,6,TOOL.skin);
  else if(cls==='axe'){const head=equipped.id==='stone_axe'?TOOL.stone:TOOL.metal;poly(ctx,[[-12,26],[3,23],[12,26],[4,36],[-11,33]],head);rect(ctx,-8,27,9,3,TOOL.metalHi);}
  else if(cls==='pickaxe'){const head=equipped.id==='stone_pickaxe'?TOOL.stone:TOOL.metal;poly(ctx,[[-13,25],[0,22],[13,25],[9,29],[0,26],[-10,29]],head);rect(ctx,-6,25,12,2,TOOL.metalHi);}
  else{ctx.strokeStyle=TOOL.metalHi;ctx.lineWidth=4;ctx.beginPath();ctx.arc(8,27,10,-1.25,.6);ctx.stroke();}
  ctx.restore();
}
...
export function drawTargetRing(ctx,node){
  ...
  ctx.strokeStyle='rgba(246,226,133,.9)';ctx.lineWidth=2;ctx.setLineDash([5,4]);ctx.beginPath();ctx.ellipse(node.x,node.y+8,node.config.radius+12,11,0,0,Math.PI*2);ctx.stroke();ctx.setLineDash([]);
}

export function drawWorldBorder(ctx,world){
  ctx.strokeStyle='#172418';ctx.lineWidth=12;ctx.strokeRect(0,0,world.width,world.height);ctx.strokeStyle='#8aa45b';ctx.lineWidth=2;ctx.strokeRect(6,6,world.width-12,world.height-12);
}
