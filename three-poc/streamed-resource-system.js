...
const HARVEST_SECONDS=1.6,RESPAWN_SECONDS=7,PLAYER_REACH=.72,CENTRAL_CLEAR_RADIUS=24,RESOURCE_EDGE_MARGIN=2.4,UI_OWNER='streamed-resource';
...
function info(type){return type==='wood'?{name:'Tree',icon:'🪵',yield:3,radius:.58}:{name:'Rock',icon:'🪨',yield:2,radius:.72};}
export function installStreamedResourceSystem({chunkManager,playerRoot,inventory}){
 ...
 function removeLoaded(id){loaded.delete(id);if(current?.id===id){current=null;progress=0;setTarget(null);if(ui?.isOwner(UI_OWNER)){ui.hideHarvest();ui.release(UI_OWNER);}}}
 ...
 function tick(now){requestAnimationFrame(tick);const dt=Math.min((now-last)/1000,.05);last=now;const moved=playerRoot.position.distanceTo(lastPos);lastPos.copy(playerRoot.position);const moving=dt>0&&moved/dt>.08;
  ...
  const hit=nearest();
  if(!hit){if(current)setTarget(null);if(ui?.isOwner(UI_OWNER))endOuterHarvest();else if(!ui){current=null;progress=0;}if(pickupTimer>0&&(pickupTimer-=dt)<=0)pickup?.classList.add('hidden');return;}
  if(ui&&!ui.isOwner(UI_OWNER))ui.acquire(UI_OWNER);
  if(current!==hit){current=hit;progress=0;setTarget(hit);}
