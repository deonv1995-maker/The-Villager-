...
const HARVEST_SECONDS=1.6,RESPAWN_SECONDS=7,RESPAWN_SAFE_DISTANCE=11,PLAYER_REACH=.72,CENTRAL_CLEAR_RADIUS=24,RESOURCE_EDGE_MARGIN=2.4,UI_OWNER='streamed-resource';
...
export function installStreamedResourceSystem({chunkManager,playerRoot,inventory,collision}){
 ...
 function removeLoaded(r){if(!r)return;loaded.delete(r.id);if(r.colliderHandle)collision?.removeCollider?.(r.colliderHandle);if(current?.id===r.id){current=null;progress=0;setTarget(null);if(ui?.isOwner(UI_OWNER)){ui.hideHarvest();ui.release(UI_OWNER);}}}
 const provider={
  createChunk({cx,cz,root,chunkSize}){const items=[],rnd=seeded(hash(cx,cz,70)),centerX=cx*chunkSize,centerZ=cz*chunkSize,count=1+Math.floor(rnd()*3),placed=[];for(let i=0;i<count;i++){let x,z,ok=false;for(let attempt=0;attempt<12&&!ok;attempt++){x=centerX+(rnd()-.5)*(chunkSize-RESOURCE_EDGE_MARGIN*2);z=centerZ+(rnd()-.5)*(chunkSize-RESOURCE_EDGE_MARGIN*2);ok=Math.abs(x)<=WORLD_LIMIT-3&&Math.abs(z)<=WORLD_LIMIT-3&&Math.hypot(x,z)>=CENTRAL_CLEAR_RADIUS&&placed.every(p=>Math.hypot(x-p.x,z-p.z)>4.5);}if(!ok)continue;const type=rnd()<.72?'wood':'stone',id=`${type}:${cx}:${cz}:${i}`,d=depleted.get(id);if(d){const safe=Math.hypot(playerRoot.position.x-d.x,playerRoot.position.z-d.z)>=RESPAWN_SAFE_DISTANCE;if(d.until>performance.now()||!safe)continue;depleted.delete(id);}const meta=info(type),visual=type==='wood'?makeTree(.68+rnd()*.34):makeRock(.68+rnd()*.3);visual.position.set(x-centerX,0,z-centerZ);root.add(visual);const r={id,type,x,z,visual,chunk:{cx,cz},...meta};if(collision?.registerCircle)r.colliderHandle=collision.registerCircle({id:`resource:${id}`,x,z,r:meta.collider,dynamicVisible:()=>visual.visible&&!!visual.parent});items.push(r);loaded.set(id,r);placed.push({x,z});}return items;},
  disposeChunk({state}){for(const r of state||[])removeLoaded(r);}
 };
 ...
 function endOuterHarvest(){current=null;progress=0;setTarget(null);if(ui){if(ui.isOwner(UI_OWNER)){ui.hideHarvest();ui.release(UI_OWNER);}}else harvestPanel?.classList.add('hidden');}
 function tick(now){requestAnimationFrame(tick);const dt=Math.min((now-last)/1000,.05);last=now;const moved=playerRoot.position.distanceTo(lastPos);lastPos.copy(playerRoot.position);const moving=dt>0&&moved/dt>.08;
  for(const [id,d] of [...depleted]){if(d.until>now)continue;if(Math.hypot(playerRoot.position.x-d.x,playerRoot.position.z-d.z)<RESPAWN_SAFE_DISTANCE)continue;depleted.delete(id);chunkManager.rebuildChunk(d.cx,d.cz,provider);}
  const hit=nearest();if(!hit){if(current)setTarget(null);if(ui?.isOwner(UI_OWNER))endOuterHarvest();else if(!ui){current=null;progress=0;}if(pickupTimer>0&&(pickupTimer-=dt)<=0)pickup?.classList.add('hidden');return;}
  ...
  if(progress>=HARVEST_SECONDS){inventory.add(hit.type,hit.yield);hit.visual.visible=false;if(hit.colliderHandle)collision?.removeCollider?.(hit.colliderHandle);depleted.set(hit.id,{until:now+RESPAWN_SECONDS*1000,x:hit.x,z:hit.z,cx:hit.chunk.cx,cz:hit.chunk.cz});showPickup(`+${hit.yield} ${hit.icon}`);chunkManager.rebuildChunk(hit.chunk.cx,hit.chunk.cz,provider);endOuterHarvest();}
