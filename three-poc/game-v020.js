...
import { installEnvironmentVisuals } from './environment-visuals.js?v=036';

await import('./game-v014.js?v=036-runtime');
...
if(version)version.textContent='3D-0.3.6';
...
if(!playerRoot){if(badge)badge.textContent='ROOT?';console.warn('[The Villager] Player root hook unavailable; gameplay fallback remains active.');}
 ...
 const visual=new GlbPlayerVisual({playerRoot,fallbackObjects,modelUrl:`${PLAYER_GLB_CONTRACT.preferredPath}?v=036`,targetHeight:PLAYER_GLB_CONTRACT.targetHeight,localGroundOffset:-0.53});
