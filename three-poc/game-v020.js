...
import { GlbPlayerVisual, PLAYER_GLB_CONTRACT } from './player-visual-glb.js?v=029';

await import('./game-v014.js?v=029-runtime');

const badge=document.querySelector('.badge');
const version=document.getElementById('version');
const harvestPanel=document.getElementById('harvest');
const playerRoot=globalThis.__villagerPlayerRoot||null;
if(version)version.textContent='3D-0.2.9';
...
if(!playerRoot){
  ...
  const visual=new GlbPlayerVisual({playerRoot,fallbackObjects,modelUrl:`${PLAYER_GLB_CONTRACT.preferredPath}?v=029`,targetHeight:PLAYER_GLB_CONTRACT.targetHeight,localGroundOffset:-0.53});
