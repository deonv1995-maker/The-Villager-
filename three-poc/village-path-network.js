...
export class VillagePathNetwork{
 ...
 connect(fromId,toId,{width=1.7,bend=.16}={}){if(this.connections.some(c=>(c.fromId===fromId&&c.toId===toId)||(c.fromId===toId&&c.toId===fromId)))return this;const a=this.nodes.get(fromId),b=this.nodes.get(toId);if(!a||!b)return this;const mid=a.entrance.clone().lerp(b.entrance,.5),dx=b.entrance.x-a.entrance.x,dz=b.entrance.y-a.entrance.y,len=Math.hypot(dx,dz)||1,nx=-dz/len,nz=dx/len;mid.x+=nx*len*bend;mid.y+=nz*len*bend;stonePath(this.root,a.entrance,mid,b.entrance,width);this.connections.push({fromId,toId});return this;}
 connectNearest(id,{width=1.55,bend=.13}={}){const node=this.nodes.get(id);if(!node)return this;let nearest=null,dist=Infinity;for(const candidate of this.nodes.values()){if(candidate.id===id)continue;const d=node.entrance.distanceTo(candidate.entrance);if(d<dist){dist=d;nearest=candidate;}}if(nearest)this.connect(id,nearest.id,{width,bend});return this;}
}
