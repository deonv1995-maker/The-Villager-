...
export function installLogConstructionSystem({world,playerRoot}){
 ...
 function fellTree(resource,count=3){const visual=createFelledTreeVisual(resource.scale||1),group=new THREE.Group();group.position.set(resource.x,0,resource.z);group.add(visual);world.add(group);const awayX=resource.x-playerRoot.position.x,awayZ=resource.z-playerRoot.position.z;group.rotation.y=Math.atan2(awayX,awayZ);let elapsed=0,last=performance.now();function animateFall(now){const dt=Math.min((now-last)/1000,.05);last=now;elapsed+=dt;const t=Math.min(1,elapsed/FALL_SECONDS),ease=1-Math.pow(1-t,3);visual.rotation.x=ease*(Math.PI/2-.05);visual.position.y=Math.sin(ease*Math.PI)*.05;if(t<1){requestAnimationFrame(animateFall);return;}world.remove(group);group.traverse(o=>{o.geometry?.dispose?.();});spawnTreeLogs(resource,count);}requestAnimationFrame(animateFall);}
 ...
 const api={logs,spawnLog,spawnTreeLogs,fellTree,pickup,drop,toggleCarry,get carried(){return carried;},get nearest(){return nearestGroundLog();}};globalThis.__villagerLogConstruction=api;return api;
}
